...
- Produces: `cloud-init.yaml` — a cloud-config that `write_files` the `provision.env`
  ...
  `context=root` → direct invocation and `context=user` → `sudo -iu "${ssh_user}"`,
  finishing with a readiness assertion (`cloud-init status --wait` is implicit; the
  module-side synthesis checks the fingerprint marker — see Task 4).
- Marker syntax in the skel: `# @@PROVISION_FILES@@` (expands to the `write_files`
  ...
  `runcmd` lines with the context mapping).
...
runcmd:
  - [ mkdir, -p, /etc/vergil ]
# @@PROVISION_RUNCMD@@
  - [ bash, -c, "cloud-init status --wait || true" ]
```
