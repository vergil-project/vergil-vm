#!/usr/bin/env bash
# tests/check-cloud-init-generation.sh — Assert all committed cloud-init.yaml files are
...
if "${ROOT}/scripts/build-cloud-init.sh" --check; then
  echo "PASS: all provider vm cloud-init.yaml files are up to date with provision scripts"
else
